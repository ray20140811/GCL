...
{ char filename[MAXPATHLEN],tmpfile[MAXPATHLEN];
 ...
 AGAIN:   
   ...
   LD_COMMAND(command,
    ...
    if(symbol_value(Vload_verbose)==Vload_verbose)
      { printf("%s\n",command); fflush(stdout);}

   {FILE *fp1 = fopen(tmpfile,"r");
    if (fp1==0) {FEerror("Couldn't open tmpfile");}
  ...
  call_init(init_address,memory,read_fasl_vector(faslfile));
